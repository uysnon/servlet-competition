...
public class EditAccountByAdminCommand implements Command {
    ...
    public EditAccountByAdminCommand() {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ValidationUtils.validate(Stream.of(passwordValidationResult, repeatPasswordValidationResult).collect(Collectors.toList()))) {
            ...
            String url = UrlUtils.getCommandUrl(CommandEnum.SHOW_EDIT_ACCOUNT_BY_ADMIN.name().toLowerCase());
            url = String.format("%s&id=%d", url, userId);
            ...
            new ShowEditAccountByAdminPageCommand().execute(request, response);
        }
